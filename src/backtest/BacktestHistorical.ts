...
export class BacktestHistorical {
    ...
    public static async getHistory(interval:BacktestInterval, startDate:BacktestDate, endDate:BacktestDate = BacktestDate.today()):Promise<BacktestSummary[]> {
        ...
        let historicalBucketPromises:Array<Promise<HistoricalBucketArray[]>> = []
        products.forEach((product:string, index:number) => {
            for (let i = 0; i < datesForHistory.length - 1; i++) {
                historicalBucketPromises.push(new Promise((resolve, reject) => {
                    try {
                        setTimeout(() => {
        ...
        let historicalBuckets:HistoricalBucketArray[][] = await Promise.all(historicalBucketPromises)
